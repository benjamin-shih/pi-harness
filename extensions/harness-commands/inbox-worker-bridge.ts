...
function asyncIdFromResponse(data: unknown): { requestId?: string; asyncId?: string; asyncDir?: string; isError?: boolean; errorText?: string } {
	if (!data || typeof data !== "object") return {};
	const raw = data as { requestId?: unknown; isError?: unknown; errorText?: unknown; result?: { details?: { asyncId?: unknown; runId?: unknown; asyncDir?: unknown } } };
	return {
		...
		errorText: textOf(raw.errorText),
	};
}
...
export function createInboxWorkerBridge(pi: ExtensionAPI) {
	...
	bus?.on?.(SLASH_SUBAGENT_RESPONSE_EVENT, (data: unknown) => {
		...
		if (!waiter) return;
		if (response.isError) {
			waiter({ state: "failed", message: response.errorText || "subagent bridge returned an error" });
	...
	bus?.on?.(SUBAGENT_ASYNC_COMPLETE_EVENT, (data: unknown) => {
		...
		void (async () => {
			const runComplete = async (summaryFile?: string) => {
				const args = ["--item-id", launch.itemId, "--status", complete.success === false ? "failed" : "completed"];
				if (summaryFile) args.push("--summary-file", summaryFile);
				if (complete.artifactPath) args.push("--artifact", complete.artifactPath);
				args.push("--json");
				await runScript(pi, "inbox-worker-complete.sh", args, launch.ctx.cwd, 8_000);
			};
	...
	async function launch(ctx: ExtensionContext, itemId: string, spec: InboxLaunchSpec): Promise<LaunchResult> {
		if (!bus?.emit || !bus?.on) {
			const result: LaunchResult = { state: "degraded", message: "pi-subagents event bridge is unavailable" };
			await recordLifecycleStatus(pi, ctx, itemId, "failed", `Worker launch did not start: ${result.message}`);
			return result;
		}
		if (spec.backend !== "pi_subagents_async") {
			const result: LaunchResult = { state: "degraded", message: `unsupported worker backend: ${spec.backend || "unknown"}` };
			await recordLifecycleStatus(pi, ctx, itemId, "failed", `Worker launch did not start: ${result.message}`);
			return result;
		}
		...
		const result = await new Promise<LaunchResult>((resolve) => {
			const timer = setTimeout(() => {
				...
				resolve({ state: "degraded", message: "timed out waiting for pi-subagents bridge response" });
			}, RESPONSE_TIMEOUT_MS);
		...
		const started = await runScript(pi, "inbox-worker-start.sh", ["--item-id", itemId, "--backend", spec.backend || "pi_subagents_async", "--runtime", "pi", "--session", ctx.sessionManager?.getSessionId?.() || "", ...(result.asyncDir ? ["--async-dir", result.asyncDir] : []), "--json"], ctx.cwd, 8_000);
		if (started.code !== 0) {
			...
			return { ...result, state: "degraded", message: "worker launch accepted, but lifecycle start recording failed" };
		}
		return result;
	}
