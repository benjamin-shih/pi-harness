...
type TaskRetentionResult = { task_api_version?: number; scope: "project" | "all"; project_scoped?: boolean; policy: { destructive_actions: boolean; archive_supported?: boolean }; summary: Record<string, number> };
function retentionLines(payload: TaskRetentionResult): string[] {
	...
	const packages = payload.project_scoped ? `${s.task_packages_scoped} scoped` : `${s.task_packages_total} total`;
	const archived = payload.project_scoped ? `${s.archived_task_packages_scoped ?? 0} archived scoped` : `${s.archived_task_packages_total ?? 0} archived total`;
	return [
