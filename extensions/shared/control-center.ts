...
async function load(){
  ...
  cards.innerHTML = [
    ...
    section('HTML artifacts', (html.modes || []).length ? '<p>Modes: ' + esc((html.modes || []).map(m => m.id).join(', ')) + '</p><p>Template: ' + esc(((html.template || {}).path) || ((html.template || {}).id) || 'none') + '</p><p>Components: ' + esc((((html.template || {}).allowed_components) || []).slice(0,8).join(', ') || 'none') + '</p><p>Publish: ' + esc(html.publish_policy || 'explicit_only') + ' · source: ' + esc(html.source_of_truth || 'json_or_markdown') + '</p><p>Auto-open: ' + esc(((html.auto_open || {}).enabled) ? 'enabled' : 'disabled') + '</p><p>Safety: ' + esc((html.safety || []).slice(0,6).join(', ')) + '</p>' : '<p class="muted">No HTML artifact recommendation.</p>'),
