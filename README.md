...
When enabled, tool rows stay terse but informative: bash calls include the command summary, reads include file/range, writes include file/line count, and edits include file/replacement count. Result badges keep Pi's green/red success/failure backgrounds and bash failures show parsed exit codes without dumping read/write/edit/bash output into the terminal view. Large bash results are also compacted before they enter model context: the harness stores captured output in a private local temp file, or points at Pi's built-in full-output file when the bash tool already created one, then returns bounded metadata plus a hard-capped visible-output tail. If Pi already truncated the tool output before the hook, the compacted result says so rather than claiming the visible output is complete.
